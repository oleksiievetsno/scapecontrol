...
| [`omezarr_lightsheet.ipynb`](omezarr_lightsheet.ipynb) | notebook | Single-timepoint LightSheetManager acquisition → GPU deskew in memory → OME-Zarr (`OMEZarrImage` / `OMEZarrMultiscale` API). |
| [`omezarr_overview.ipynb`](omezarr_overview.ipynb) | notebook | Whole-well tiled brightfield Z-stack, assembled in memory and saved as OME-Zarr / OME-TIFF. |
| [`brightfield_overview.ipynb`](brightfield_overview.ipynb) | notebook | Acquire a tiled brightfield overview, stitch it into `mosaic.tif`, and save `overview.ome.zarr`. |
